...
from processing import get_debt_report

class DebtReportForm:
    def __init__(self, parent, db):
        ...
        # Treeview for report
        self.tree = ttk.Treeview(parent, columns=('STT', 'Customer Name', 'Beginning Debt', 'Transactions', 'Ending Debt'), show='headings')
        self.tree.heading('STT', text='STT')
        self.tree.heading('Customer Name', text='Khách hàng')
        self.tree.heading('Beginning Debt', text='Nợ đầu')
        self.tree.heading('Transactions', text='Phát sinh')
        self.tree.heading('Ending Debt', text='Nợ cuối')
    ...
    def generate_report(self):
        try:
            ...
            return
        report = get_debt_report(self.db, year, month)
        for child in self.tree.get_children():
            self.tree.delete(child)
        for i, item in enumerate(report, start=1):
            self.tree.insert('', 'end', values=(i, item['customer_name'], item['beginning_debt'], item['transactions'], item['ending_debt']))